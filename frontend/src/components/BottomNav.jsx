    ...
    return (
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-slate-900/95 backdrop-blur-xl border-t border-white/10 px-10 py-3 pb-safe z-50">
        <div className="flex justify-between items-center text-slate-400">
          {adminNav.map((item) => {
            // Check if active based on path and search params
            const isActive = path === item.path && (location.search === item.query || (!location.search && item.name === 'Cases'));
            const Icon = item.icon;
            return (
              <NavLink key={item.name} to={`${item.path}${item.query}`} className={`flex flex-col items-center space-y-1 ${isActive ? 'text-white' : 'hover:text-white'}`}>
                <Icon size={20} />
    ...
    return (
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-indigo-950/95 backdrop-blur-xl border-t border-white/10 px-10 py-3 pb-safe z-50">
        <div className="flex justify-between items-center text-white/60">
          {superNav.map((item) => {
            const isActive = path === item.path && (location.search === item.query || (!location.search && item.name === 'Regions'));
            const Icon = item.icon;
            return (
              <NavLink key={item.name} to={`${item.path}${item.query}`} className={`flex flex-col items-center space-y-1 ${isActive ? 'text-white' : 'hover:text-white'}`}>
